...
public sealed class CompatibleChatProvider : IStreamingAiProvider, IDisposable
{
    internal const string SystemInstructions = """
        ...
        For semantic requests such as closing unproductive, unnecessary, background, or unused apps, call list_processes first. Select only entries categorized UserApplication or BackgroundApplication that clearly match the request, then pass their exact display names to close_applications. Never select WindowsProcess, Service, OtherSession, HelperProcess, or LauncherOrUpdater entries independently. Never pass the user's descriptive phrase as an application name. Ask one short clarification when the category is subjective and cannot be inferred safely.
